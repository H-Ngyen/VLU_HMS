...
// Component phụ tách ra ngoài để tối ưu performance
const InfoRow = ({ label, value, fullWidth = false, className = "" }: InfoRowProps) => (
...
const EditRecord = () => {
  ...
  useEffect(() => {
    if (recordToEdit) {
      let transfers = recordToEdit.managementData?.transfers || [];
      if (transfers.length === 0) {
        transfers = [
          {
            department: recordToEdit.department || "",
            date: recordToEdit.admissionDate || "",
            time: "",
            days: "",
          },
        ];
      ...
      const initializedData: Record = {
        ...recordToEdit,
        managementData: {
          ...recordToEdit.managementData,
          transfers: transfers,
        },
        diagnosisInfo: recordToEdit.diagnosisInfo || {
        ...
        dischargeStatusInfo: recordToEdit.dischargeStatusInfo || {
          treatmentResult: "", // 1..5
          pathology: "", // 1..3
        ...
        medicalRecordContent: {
          ...recordToEdit.medicalRecordContent,
        },
      };

      // Deep merge existing data for nested objects in medicalRecordContent if necessary
      // Since recordToEdit.medicalRecordContent is fully typed, spread should be enough.
      // But let's ensure sub-objects are not missing if recordToEdit has them missing (though types say they are required)
      
      // If we assume types.ts reflects reality, we don't need manual merges unless optional fields.
      // However, to be safe and matching previous logic:
      if (recordToEdit.medicalRecordContent?.relatedCharacteristics) {
        initializedData.medicalRecordContent.relatedCharacteristics = {
          ...initializedData.medicalRecordContent.relatedCharacteristics,
          ...recordToEdit.medicalRecordContent.relatedCharacteristics,
        };
      }
      if (recordToEdit.medicalRecordContent?.vitalSigns) {
        initializedData.medicalRecordContent.vitalSigns = {
          ...initializedData.medicalRecordContent.vitalSigns,
          ...recordToEdit.medicalRecordContent.vitalSigns,
        };
      }
      if (recordToEdit.medicalRecordContent?.organs) {
        initializedData.medicalRecordContent.organs = {
          ...initializedData.medicalRecordContent.organs,
          ...recordToEdit.medicalRecordContent.organs,
        };
      }
      if (recordToEdit.medicalRecordContent?.admissionDiagnosis) {
        initializedData.medicalRecordContent.admissionDiagnosis = {
          ...initializedData.medicalRecordContent.admissionDiagnosis,
          ...recordToEdit.medicalRecordContent.admissionDiagnosis,
        };
      }
  ...
  if (!recordToEdit)
    return <div className="p-8 text-center">Hồ sơ không tồn tại</div>;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-orange-100 rounded px-3 py-2 text-gray-700 font-medium"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-orange-600 uppercase mb-1">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-orange-100 rounded px-3 py-2 text-gray-700 font-mono"
                    />
                  </div>
                </div>
              )}
                          ...
                          <tr key={v.f}>
                            ...
                            <td className="border p-2">
                              <input
                                ...
                                onChange={(e) =>
                                  handleVitalSignsChange(v.f, e.target.value)
                                }
                              />
                            </td>
                    ...
                    {organFields.map((field) => (
                      <div key={field.key} className="flex flex-col">
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            handleOrgansChange(field.key, e.target.value)
                          }
                        />
                      </div>
                    ))}
