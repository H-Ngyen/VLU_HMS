...
public class NotificationTemplateContext
{
    ...
    public string StorageCode { get; init; } = null!;
}
public class NotificationTemplates(IConfiguration config)
{
    ...
    private NotificationTemplates BuildHematologyIntitial(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Có yêu cầu xét nghiệm máu mới cho bệnh án #{ctx.StorageCode} (Mã phiếu: #{ctx.ResourceId}).";

        var url = GenerateHematologyLink(ctx.ResourceId, ctx.StorageCode);
        var emailTitle = $"[VLU_HMS] Thông báo yêu cầu xét nghiệm máu mới - Bệnh án #{ctx.StorageCode}";
        var emailContent = $@"
            <h3>Yêu cầu xét nghiệm máu mới</h3>
            <p>Hệ thống ghi nhận một yêu cầu xét nghiệm máu mới với thông tin sau:</p>
            <ul>
                <li><strong>Mã bệnh án:</strong> #{ctx.StorageCode}</li>
    ...
    private NotificationTemplates BuildHematologyReceived(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Phiếu xét nghiệm máu #{ctx.ResourceId} của bệnh án #{ctx.StorageCode} đã được tiếp nhận.";
    ...
    private NotificationTemplates BuildHematologyProcessing(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Phiếu xét nghiệm máu #{ctx.ResourceId} của bệnh án #{ctx.StorageCode} đang trong quá trình thực hiện.";
    ...
    private NotificationTemplates BuildHematologyCompleted(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Phiếu xét nghiệm máu #{ctx.ResourceId} của bệnh án #{ctx.StorageCode} đã có kết quả hoàn tất.";
    ...
    private NotificationTemplates BuildXrayIntitial(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Có yêu cầu chụp X-Quang mới cho bệnh án #{ctx.StorageCode} (Mã phiếu: #{ctx.ResourceId}).";

        var url = GenerateXrayLink(ctx.ResourceId, ctx.StorageCode);
        var emailTitle = $"[HMS] Thông báo yêu cầu chụp X-Quang mới - Bệnh án #{ctx.StorageCode}";
        var emailContent = $@"
    ...
    private NotificationTemplates BuildXrayReceived(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Phiếu X-Quang #{ctx.ResourceId} của bệnh án #{ctx.StorageCode} đã được tiếp nhận.";
    ...
    private NotificationTemplates BuildXrayProcessing(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Phiếu X-Quang #{ctx.ResourceId} của bệnh án #{ctx.StorageCode} đang được thực hiện.";
    ...
    private NotificationTemplates BuildXrayCompleted(NotificationTemplateContext ctx)
    {
        ...
        var appContent = $"Phiếu X-Quang #{ctx.ResourceId} của bệnh án #{ctx.StorageCode} đã có kết quả hoàn tất.";
